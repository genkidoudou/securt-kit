        ...
        try {
            ...
            if (dto != null) {
                ...
                if (StrUtil.isNotBlank(dto.getSourceTableName()) && StrUtil.isNotBlank(sourceColumn)) {
                    ...
                    if (cryptoService.needEncrypt(dto.getSourceTableName(), sourceColumn, this.datasourceId)) {
                        String plainToEncrypt = value;
                        if (dto.getMatchType() == ParameterMatchType.LIKE) {
                            LikeHandleResult likeResult = LikePatternHandlerHolder.getHandler().handle(
                                    value,
                                    new LikeHandleContext(dto.getSourceTableName(), sourceColumn,
                                            this.datasourceId, parameterIndex));
                            if (likeResult == null || likeResult.getAction() == LikeHandleResult.Action.SKIP) {
                                log.debug("LIKE handler skip encryption [table={}, column={}, index={}]: {}",
                                        dto.getSourceTableName(), sourceColumn, parameterIndex,
                        ...
                        String encrypted = cryptoService.encrypt(dto.getSourceTableName(), sourceColumn,
                                plainToEncrypt, this.datasourceId);
