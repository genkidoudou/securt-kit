...
import io.github.genkidoudou.core.parser.constant.NumberConstant;
...
import io.github.genkidoudou.core.parser.visitor.fieldparse.FieldParseParseTableFromItemVisitor;
...
public class PoJoEncrtptorStatementVisitor implements StatementVisitor {
    ...
    public void visit(Update update) {
        ...
        table.accept(fieldParseTableFromItemVisitor);
        //join的表（update t1 join t2 写在表名前的 startJoins，以及写在 set 之后的 joins 两种语法都要收集）
        List<Join> joins = new ArrayList<>();
        joins.addAll(Optional.ofNullable(update.getStartJoins()).orElse(new ArrayList<>()));
        joins.addAll(Optional.ofNullable(update.getJoins()).orElse(new ArrayList<>()));
        for (Join join : joins) {
            FromItem rightItem = join.getRightItem();
            if (rightItem != null) {
                rightItem.accept(fieldParseTableFromItemVisitor);
...
//                log.info("【securt-kit】UPDATE SET字段: {} = {}", column.getColumnName(), expression.toString());
        ...
        // UPDATE 的 SET 字段无论使用占位符还是字面量，都需要暴露给调用方判断是否配置了加密。
        // 实际策略由 SecurtkitUtils.parseSql(sql, datasourceId) 按数据源统一过滤和填充。
        collectUpdateEncryptFieldInfos(updateSets, NumberConstant.ONE,
                fieldParseTableFromItemVisitor.getLayerFieldTableMap(),
                extractPureTableName(table.getName()));
...
//        }
        ...
        Set<FieldInfoDto> insertFieldInfoDtos = CollectionUtil.isEmpty(columns)
                ? new HashSet<>()
                : columns.stream()
                        .map(m -> toInsertFieldInfo(m, pureTableName))
                        .collect(Collectors.toSet());

        // insert ... set 字段=值 以及 on duplicate key update 字段=值 都会显式带上列名，
        // 这些列同样属于当前表，需要一起放进表字段信息里，否则解析不到占位符所属字段
        List<UpdateSet> setUpdateSets = insert.getSetUpdateSets();
        List<UpdateSet> duplicateUpdateSets = insert.getDuplicateUpdateSets();
        Set<FieldInfoDto> fieldInfoDtos = new HashSet<>(insertFieldInfoDtos);
        collectUpdateSetFieldInfo(fieldInfoDtos, setUpdateSets, pureTableName);
        collectUpdateSetFieldInfo(fieldInfoDtos, duplicateUpdateSets, pureTableName);
        ...
        //4.没有指定字段顺序时，values 无法和字段对应，这部分不支持；
        // 但 set / on duplicate key update 是显式带列名的，仍然可以正常解析
        if (CollectionUtil.isEmpty(columns)) {
            log.warn("【securt-kit】insert 语句未指定表字段顺序，values 部分不支持自动加解密，请规范语法 原sql:{}", insert.toString());
            parseUpdateSets(setUpdateSets, NumberConstant.ONE, layerFieldTableMap);
            parseUpdateSets(duplicateUpdateSets, NumberConstant.ONE, layerFieldTableMap);
            return;
        }

        //5.处理select（insert ... set 语法下 select 为 null）
        ...
        if (select != null) {
            ...
            //解析占位符（注意：insert 语句的前后字段有对应关系，所以这里把insert前面的字段传递给后面的visitor）
        ...
        List<FieldEncryptorInfoDto> fieldInfos = insertFieldInfoDtos.stream()
        ...
        //7.insert ... set / on duplicate key update 的 列=表达式 按 update set 的方式处理
        parseUpdateSets(setUpdateSets, NumberConstant.ONE, layerFieldTableMap);
        parseUpdateSets(duplicateUpdateSets, NumberConstant.ONE, layerFieldTableMap);
    }
     ...
     * 将 insert 的列转换成表字段信息
     */
    private FieldInfoDto toInsertFieldInfo(Column column, String pureTableName) {
        String columnName = column.getColumnName().toLowerCase();
        return new FieldInfoDto(columnName, columnName, pureTableName, true);
    }

    /**
     * 把 set / on duplicate key update 里出现的列补充到表字段信息中
     */
    private void collectUpdateSetFieldInfo(Set<FieldInfoDto> fieldInfoDtos, List<UpdateSet> updateSets, String pureTableName) {
        ...
        for (UpdateSet updateSet : updateSets) {
            ...
            for (Column column : columns) {
                fieldInfoDtos.add(toInsertFieldInfo(column, pureTableName));
            }
        }
    }

    /**
     * 解析 列=表达式 形式的赋值中的占位符（update set、insert set、on duplicate key update 共用）
     */
    private void parseUpdateSets(List<UpdateSet> updateSets, int layer,
        ...
        for (UpdateSet updateSet : updateSets) {
            ...
            if (CollectionUtil.isEmpty(columns) || values == null) {
                continue;
            }
            for (int i = 0; i < columns.size() && i < values.size(); i++) {
                ...
                //处理左右两边表达式的占位符信息
                JsqlparserUtil.parseWhereColumTable(layer,
                        layerFieldTableMap,
                        columns.get(i),
                        (Expression) value,
                        this.getPlaceholderColumnTableMap());
    ...
    /**
     * 收集 UPDATE SET 左侧字段。字面量不会进入占位符映射，因此必须独立收集。
     */
    private void collectUpdateEncryptFieldInfos(
        ...
        Set<String> existing = this.fieldEncryptorInfos.stream()
                .map(info -> (String.valueOf(info.getSourceTableName()) + '\0'
                        + String.valueOf(info.getSourceColumn())).toLowerCase(Locale.ROOT))
                .collect(Collectors.toSet());
        ...
        for (UpdateSet updateSet : updateSets) {
            ...
            for (Column column : columns) {
                ColumnTableDto resolved = JsqlparserUtil.parseColumn(column, layer, layerFieldTableMap);
                String sourceTable = resolved == null ? null : resolved.getSourceTableName();
                String sourceColumn = resolved == null ? null : resolved.getSourceColumn();
                if (StrUtil.isBlank(sourceTable)) {
                    sourceTable = fallbackTableName;
                }
                if (StrUtil.isBlank(sourceColumn)) {
                    sourceColumn = column.getColumnName();
                }
                if (StrUtil.isBlank(sourceTable) || StrUtil.isBlank(sourceColumn)) {
                    continue;
                }

                String key = (sourceTable + '\0' + sourceColumn).toLowerCase(Locale.ROOT);
                if (existing.add(key)) {
                    this.fieldEncryptorInfos.add(FieldEncryptorInfoDto.builder()
    ...
    public void visit(Select select) {
        ...
        List<FieldEncryptorInfoDto> fieldInfos = selectFiles.stream()
                ...
                .map(m -> FieldEncryptorInfoDto.builder()
                        ...
                        .resultColumnIndex(m.getResultColumnIndex())
