...
public class SimpleInterceptorStatement implements Statement {
    ...
    private void logTableNames(String sql) {
        try {
            // 使用缓存优化的表名解析
            Set<String> tables = io.github.genkidoudou.core.parser.SqlParseCache.parseTableNames(sql);
            if (!tables.isEmpty()) {
    ...
    public ResultSet executeQuery(String sql) throws SQLException {
        ...
        if (io.github.genkidoudou.core.config.ConfigInitializer.shouldSkipByComment(sql)
                || !io.github.genkidoudou.core.config.EncryptModeHolder.isJdbc()) {
            log.debug("Decryption disabled for Statement query [skipComment or mode={}]",
                    io.github.genkidoudou.core.config.EncryptModeHolder.getMode());
            ResultSet resultSet = delegate.executeQuery(sql);
            long endTime = System.currentTimeMillis();
            log.info("[QUERY RESULT] Executed in " + (endTime - startTime) + "ms (skip encrypt)");
            return resultSet;
        }
        ...
        try {
            // 使用缓存优化的表名解析
            tables = io.github.genkidoudou.core.parser.SqlParseCache.parseTableNames(sql);
    ...
    public int executeUpdate(String sql) throws SQLException {
        ...
        if (io.github.genkidoudou.core.config.ConfigInitializer.shouldSkipByComment(sql)) {
            log.debug("SQL matched skip comment token for Statement update");
        } else {
            log.debug("Statement.executeUpdate does not rewrite string literals for encryption; prefer PreparedStatement");
        }
