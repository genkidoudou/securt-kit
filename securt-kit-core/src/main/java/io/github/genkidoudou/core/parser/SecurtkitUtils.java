...
import io.github.genkidoudou.core.parser.visitor.PoJoEncrtptorStatementVisitor;
...
public class SecurtkitUtils {
            ...
            for (FieldEncryptorInfoDto field : result.getValue()) {
                ...
                if (strategy != null) {
                    FieldEncryptorInfoDto rebuilt = FieldEncryptorInfoDto.builder()
                            ...
                            .resultColumnIndex(field.getResultColumnIndex())
    ...
    private static Pair<Map<String, ColumnTableDto>, List<FieldEncryptorInfoDto>> doParseSql(String sql) throws JSQLParserException {
        ...
        try {
            ...
            // 5. 获取解析结果并建立占位符索引映射
            // 注意：右值被函数包裹时 key 可能是 UPPER(SECURT_KIT_PLACEHOLDER_1)，需抽取其中的序号
            ...
            for (Map.Entry<String, ColumnTableDto> entry : placeholderColumnTableMap.entrySet()) {
                ...
                Integer index = extractPlaceholderIndex(key);
     ...
     * 从解析 key 中抽取占位符序号。
     * <p>
     * 支持裸占位符 {@code SECURT_KIT_PLACEHOLDER_1}，以及被函数包裹的形式
     * （如 {@code UPPER(SECURT_KIT_PLACEHOLDER_1)}）。
     * </p>
     *
     * @param key visitor 产出的 map key，可为 null
     * @return 1-based 序号；无法识别时返回 null
     */
    static Integer extractPlaceholderIndex(String key) {
        if (key == null || key.isEmpty()) {
            return null;
        }
        int start = key.indexOf(PLACEHOLDER);
        if (start < 0) {
            return null;
        }
        int numStart = start + PLACEHOLDER.length();
        int numEnd = numStart;
        while (numEnd < key.length() && Character.isDigit(key.charAt(numEnd))) {
            numEnd++;
        }
        if (numEnd == numStart) {
            return null;
        ...
        try {
            return Integer.parseInt(key.substring(numStart, numEnd));
