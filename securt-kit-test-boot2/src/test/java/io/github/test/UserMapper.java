...
public interface UserMapper {
    ...
    @Results(id = "`user`EntityMap", value = {
    ...
    @ResultMap("`user`EntityMap")
    List<UserEntity> selectAllEntities();

    // === Multi-table JOIN queries ===
    @Select("SELECT u.id AS uid, u.name AS uname, u.phone AS uphone, o.id AS oid, o.amount AS oamount FROM `user` u JOIN orders o ON u.id = o.`user`_id WHERE o.amount > #{min}")
    List<Map> selectUsersWithOrders(@Param("min") int min);

    @Select("SELECT u.name AS uname, o.amount AS oamount FROM `user` u LEFT JOIN orders o ON u.id = o.`user`_id WHERE u.name LIKE #{nameLike}")
