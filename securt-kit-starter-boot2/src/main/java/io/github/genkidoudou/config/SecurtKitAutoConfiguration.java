...
/**
 * Securt-Kit 自动配置：配置绑定、TableCache 初始化、FieldCryptoService、
...
public class SecurtKitAutoConfiguration {
    ...
    /**
     * MYBATIS 模式：注册 EncryptInterceptor（需 classpath 存在 MyBatis + securt-kit-mybatis）
     */
    @Configuration
    ...
    private static void validateMode(FieldEncryptorProperties.Mode mode,
        ...
        if (mode == FieldEncryptorProperties.Mode.MYBATIS) {
            ...
            List<DataSource> list = dataSourceProvider.stream().collect(java.util.stream.Collectors.toList());
            if (list.isEmpty()) {
            ...
            for (DataSource ds : list) {
                ...
                // also check driver class via metadata if possible
    ...
    private static String resolveJdbcUrl(DataSource dataSource) {
        ...
        try {
            // Spring Boot common: getJdbcUrl / getUrl
